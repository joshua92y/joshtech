...
function Table({ data, onRowClick }: TableProps) {
  ...
  const headers = data.headers.map((header, index) => (
    <th
      ...
      className="font-label font-default font-s px-16 py-12"
      key={index}
    >
  ...
  const rows = (sortConfig ? sortedRows : data.rows).map((row, index) => (
    ...
    >
      {row.map((cell, cellIndex) => (
        <td className="font-body font-default font-s px-16 py-12" key={cellIndex}>
          {cell}
        </td>
      ))}
    </tr>
      ...
      >
        ...
        <tbody className="neutral-on-background-medium">
          {rows.length > 0 ? (
            ...
            <tr>
              <td colSpan={headers.length} className="font-body font-default font-s px-24 py-12">
                No data available
              </td>
            </tr>
          )}
